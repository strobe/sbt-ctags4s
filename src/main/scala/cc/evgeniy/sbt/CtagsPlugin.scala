...
ent location */
  ...
  def updateCtags(baseDirectory: File): Unit = {
    ...
    val filesArgs = (files.filter(!_.toFile.isHidden)).map(_.toString)
    val dirsArgs = (dirs.filter(!_.toFile.isHidden)).map(_.toString)
    val args = Array("-f", "TAGS", "-R", "-e", "*") ++ dirsArgs ++ filesArgs
